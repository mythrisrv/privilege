...
router.post(
  "/create",
  [validationMiddleware.createCustomervalidator, jwtauth],
  async (req, res) => {
    try {
...
router.get("/visitLog", [jwtauth], async (req, res) => {
  try {
    let item = await customerVisitLog(req);
    res.status(200).json({
      status: 200,
      data: item,
    });
...
router.get("/receipts", [jwtauth], async (req, res) => {
  try {
    let item = await customerReceipts(req);
    res.status(200).json({
      status: 200,
      data: item,
    });
...
router.get(
  ...
  /* [jwtauth],*/ async (req, res) => {
    try {
      let item = await customerInvoice(req);
      res.status(200).json({
        status: 200,
        data: item,
      });
...
router.get(
  ...
  /* [jwtauth],*/ async (req, res) => {
    try {
      let item = await customerStatement(req);
      res.status(200).json({
        status: 200,
        data: item,
      });
