          ...
          if (companyname.length > 0) {
            companyname.forEach((data) => {
              ...
              var cust_reg_number =
                company_shortcode +
                localbodyName +
                customer_id.toString().padStart(6, "0");
              ...
              Customer.save().then(async (result) => {
                ...
                if (packageDetails) {
                  packageDetails.forEach(async (data) => {
                    ...
                    invoice.save().then(async (result) => {
                      ...
                      let numberOfInvoiceItem =
                        await models.InvoiceItem.countDocuments();
                ...
                let numberOfThariff =
                  await models.TariffAssign.countDocuments();
                var thariff_id = numberOfThariff + 1;
                thariff.tariff_assign_id = thariff_id;
                var tariff_regnumber =
                  "TF" + thariff_id.toString().padStart(5, "0");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let customerProfile = await models.Customer.findOne({
        ...
        .populate(
          "cust_package_id",
          "package_name package_reg_fee package_basic_fee"
        )
        .select(
          "cust_name cust_phone cust_landline_no cust_watsapp_no cust_image cust_verification_status cust_serial_no cust_designation cust_no_members cust_house_num cust_address cust_address1 cust_billing_type"
        );
      if (customerProfile) {
        resolve(customerProfile);
      } else {
...
customerVisitLog = (req) => {
  let customer_id = req.query.id;
  return new Promise(async (resolve, reject) => {
    try {
      let visitLog = await models.WasteCollect.find({
        $and: [{ waste_clt_status: 0 }, { waste_clt_cust_id: customer_id }],
      })
        .populate("waste_clt_addedby", "username")

        .select("waste_clt_date waste_clt_time");
      if (visitLog) {
        resolve(visitLog);
      } else {
        reject({ message: "Customer not exist" });
...
customerReceipts = (req) => {
  let customer_id = req.query.id;
  return new Promise(async (resolve, reject) => {
    try {
      let receipt = await models.Receipt.find({
        $and: [{ receipt_status: 0 }, { receipt_cust_id: customer_id }],
      })
        .populate("receipt_addedby", "username")
        .populate("receipt_cust_id", "cust_name")

        .select(
          "receipt_date receipt_no receipt_amount receipt_due_amt receipt_time  "
        )
        .sort({ receipt_date: -1 })
        .limit(20);
      if (receipt) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        let invoice= await models.Invoice.aggregate([
          {
            $match: {invoice_status: "0",invoice_customer_id:customer_id}
          },
          {
            $project:{
              date:"$invoice_date"
